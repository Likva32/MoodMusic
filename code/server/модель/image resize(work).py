...
class ChooseSaveDialog(wx.Dialog):
    ...
    def save_orig(self, evt):
        save_dialog = wx.FileDialog(self, "Save Original to JPEG", "", "", "JPG files(*.jpg)",
    ...
    def save_roi(self, evt):
        save_dialog = wx.FileDialog(self, "Save ROI to JPEG", "", "", "JPG files(*.jpg)",
                                    wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)  # |wx.FD_CHANGE_DIR)
...
class ShowCapture(wx.Frame):
    def __init__(self, capture, fps=60):
        ...
        self.roi_x = wx.SpinCtrl(panel, -1, "ROI X", style=wx.TE_PROCESS_ENTER | wx.SP_ARROW_KEYS, min=0, max=width,
                                 initial=0, size=(60, -1))
        self.roi_y = wx.SpinCtrl(panel, -1, "ROI Y", style=wx.TE_PROCESS_ENTER | wx.SP_ARROW_KEYS, min=0, max=height,
                                 initial=0, size=(60, -1))
        self.roi_width = wx.SpinCtrl(panel, -1, "ROI W", style=wx.TE_PROCESS_ENTER | wx.SP_ARROW_KEYS, min=0, max=width,
                                     initial=width, size=(60, -1))
        self.roi_height = wx.SpinCtrl(panel, -1, "ROI H", style=wx.TE_PROCESS_ENTER | wx.SP_ARROW_KEYS, min=0,
                                      max=height, initial=height, size=(60, -1))
        ...
        if os.path.isfile(settings_bmp_path):
            ...
            settings_button = wx.BitmapButton(panel, wx.ID_ANY, bitmap=settings_bmp, style=wx.NO_BORDER,
                                              size=(32, 32))  # )
        else:
                    ...
                    try:
                        ...
                        if hasattr(self, 'roi_click_down_pos'):
                            ...
                            if cross_y > roi_y1:
                                ...
                                frameRoi[cross_y:roi_height, 0:roi_width] = frameRoi[cross_y:roi_height,
                                                                            0:roi_width] * .50
                            else:
                                frameRoi[roi_y1:roi_height, 0:roi_width] = frameRoi[roi_y1:roi_height,
                                                                           0:roi_width] * .50
                            ...
                            if cross_x > roi_x1:
                                ...
                                frameRoi[0:roi_height, cross_x:roi_width] = frameRoi[0:roi_height,
                                                                            cross_x:roi_width] * .50
                            else:
                                frameRoi[0:roi_height, roi_x1:roi_width] = frameRoi[0:roi_height,
                                                                           roi_x1:roi_width] * .50
